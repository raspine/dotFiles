...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "h",
        function()
			if awful.layout.get(client.focus.screen) == lain.layout.termfair then
				awful.client.focus.bydirection("left")
			elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.left then
				-- We want to focus the previous focused slave window
				lastFocus = awful.client.focus.history.previous()
				awful.client.focus.bydirection("left", lastFocus)
			elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.right then
    ...
    awful.key({ modkey }, "l",
        function()
			...
			elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.right then
					-- We want to focus the previous focused slave window
					lastFocus = awful.client.focus.history.previous()
					awful.client.focus.bydirection("right", lastFocus)
			end
            if client.focus then client.focus:raise() end
        end),
    awful.key({ modkey }, "k",
	...
	-- Change termfair columns
    awful.key({ altkey,    }, "+",      function () 
											lain.layout.termfair.nmaster = lain.layout.termfair.nmaster + 1
											awful.layout.arrange(client.focus.screen)
										end),
    awful.key({ altkey,    }, "-",      function ()
											if (lain.layout.termfair.nmaster > 0) then
												lain.layout.termfair.nmaster = lain.layout.termfair.nmaster - 1
												awful.layout.arrange(client.focus.screen)
											end
										end),
    ...
    awful.key({ modkey, "Shift"   }, "h", function ()
											...
											elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.left then
												-- We aim to shift left but this command always puts the slave window 
												...
												-- window and shift this left, then we focus back so our focus 
												-- follows the window.
												awful.client.focus.history.previous()
												awful.client.swap.bydirection("right")
												--awful.client.focus.history.previous()
											elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.right then
												-- Straight forward we shift with the master if we can
												awful.client.swap.bydirection("left")
											end
											end),
    awful.key({ modkey, "Shift"   }, "l", function ()
											...
											elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.left then
												awful.client.swap.bydirection("right")
											elseif awful.layout.get(client.focus.screen) == lain.layout.uselesstile.right then
												-- We aim to shift right but this command always puts the slave window 
												-- in the top of the slave column. So to keep the order of our slave
												-- windows intact we we use history buffer and focus the last slave
												-- window and shift this left, then we focus back so our focus 
												-- follows the window.
